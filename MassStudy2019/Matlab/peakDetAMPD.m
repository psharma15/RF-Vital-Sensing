...
function [minimaIdx, maximaIdx] = peakDetAMPD(signal,  fs,opts)
% Input: 
%       signal: Input signal, 1-D array
%       segFreq: Frequency of the segment, can be average frequency
%       expected. If too low, may miss quick changes.
...
if ~isfield(opts,'freqSegPkDet')
    opts.freqSegPkDet = 15/60;
    fprintf('Looking for peaks over 1 cycle(s) @%d BPM \n',opts.freqSegPkDet*60);
end

signal = signal(:); % Make sure that signal is a column vector
nCyclesPerSegment = 5; % Original is 1 cycle/segment
...
% *********************************************************************** %
n = ceil(nCyclesPerSegment*fs/opts.segFreq);
lenSegment = 2*n + 1;
lenOverlap = n + 1;

% ------------------------------------------------------------------------
% Padding the signal at the starting and end so that peak detection is
...
%     numScaleMax = ceil(fs/freqRange(1));
    ...
    maximaIdx = [maximaIdx; maximaPtUpdated]; %#ok<*AGROW>
