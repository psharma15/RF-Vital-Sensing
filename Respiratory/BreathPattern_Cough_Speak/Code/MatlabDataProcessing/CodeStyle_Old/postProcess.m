...
if (dataRow < 1) || (dataCol ~=2)
    fprintf('In post-processing, input data is expected in [amp,ph] format.');
    return;
end
...
% Low pass filter 
filtLP = fdesign.lowpass('Fp,Fst,Ap,Ast',fp,fst,0.1,80,fs); % Change Ast from 80 to 10 if filtfilt creates error
